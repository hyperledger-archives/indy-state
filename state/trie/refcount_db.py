...
class RefcountDB:
    ...
    def dec_refcount(self, k):
        ...
        self.journal.append([node_object[0], k])
        new_refcount = utils.encode_int(refcount - 1)
        self._keyValueStorage.put(b'r:'+k, rlp.encode([new_refcount, node_object[1]]))
    ...
    def cleanup(self, epoch):
        try:
            death_row_node = self._keyValueStorage.get('deathrow:'+str(epoch))
        except:
        ...
        for nodekey in death_row_nodes:
            try:
                refcount, val = rlp.decode(self._keyValueStorage.get(b'r:'+nodekey))
    ...
    def commit_refcount_changes(self, epoch):
        ...
        try:
            death_row_nodes = rlp.decode(self._keyValueStorage.get('deathrow:'+str(timeout_epoch)))
        except:
            death_row_nodes = []
        for nodekey in self.death_row:
            ...
            if refcount == ZERO_ENCODED:
                ...
                self._keyValueStorage.put(b'r:'+nodekey, rlp.encode([new_refcount, val]))
        ...
        # Save journal
        try:
            journal = rlp.decode(self._keyValueStorage.get('journal:'+str(epoch)))
        except:
            journal = []
    ...
    def revert_refcount_changes(self, epoch):
        ...
        try:
            ...
            for new_refcount, hashkey in journal[::-1]:
                node_object = rlp.decode(self._keyValueStorage.get(b'r:'+hashkey))
                self._keyValueStorage.put(b'r:'+hashkey,
